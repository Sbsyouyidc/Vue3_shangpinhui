      ...
      <div class="fl value">
        <ul class="type-list">
          <li 
            ...
            :key="index"
            @click="$emit('attrsInfo', attrs, attrValue)"
          >
            <a>{{attrValue}}</a>
          </li>
        </ul>
...
// 获取trademark
const trademarkList = computed<Trademark[]>(
  mapGetters(['trademarkList']).trademarkList.bind({$store: store})
)

// 获取attrsList
const attrsList = computed<Attrs[]>(
  mapGetters(['attrsList']).attrsList.bind({$store: store})
)

const emits = defineEmits<{
  (e: 'trademarkInfo', trademark: Trademark),
  (e: 'attrsInfo', attrs: Attrs, attrValue: string)
}>()
