            ...
            <tr>
              <th colspan="5">
                <span class="ordertitle">{{order.createTime}}　订单编号：{{order.outTradeNo}} 
                  <span class="pull-right delete"><img :src="order.imgUrl"></span>
                </span>
              </th>
            </tr>
          </thead>
...
import { MyOrder, Record, Detail } from './types'
...
async function getData() {
  try {
    let res = await API.reqMyOrder(data.page, data.limit)
    if (res.code === 200) {
      myOrder.records = res.data.records
      myOrder.total = res.data.total
      myOrder.size = res.data.size
      myOrder.current = res.data.current
      myOrder.pages = res.data.pages
    } else {
      console.log(res)
    }
  } catch (error) {}
...
function getPageNo(page: number) {
